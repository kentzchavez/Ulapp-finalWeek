...
def gen_preci_temp(weather_data):
    ...
    for date, forecast_data in daily_forecast.items():
        ...
        norm_temperatures = (np.array(temperatures) - min(temperatures)) / (max(temperatures) - min(temperatures))

        # Calculate circle sizes based on probability of precipitation
        # Normalize the precipitation for circle sizes
        norm_precipitation = (np.array(precipitation) - min(precipitation)) / (max(precipitation) - min(precipitation))
...
# Example usage:
# Call this function with the weather data as an argument
# generate_temperature_scatter_plot(weather_data)
